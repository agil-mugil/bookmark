...
public interface GroupReferenceRepository extends JpaRepository<GroupReference, GroupReferenceId> {
		...
		@Query(value = "select * from tgrouprefdataeng gdata where gdata.GROUPTYPE=?1 AND gdata.groupvalue not in (select distinct g.GROUPVALUE from tbookmarkgroupseng g)", nativeQuery = true)
